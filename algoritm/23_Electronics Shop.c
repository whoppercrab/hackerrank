 ...
 */
 void sorting(int rev,int* arr,int cnt)
 {
     int temp=0;
...
int getMoneySpent(int keyboards_count, int* keyboards, int drives_count, int* drives, int b) {
     ...
     */
    sorting(1,keyboards,keyboards_count);
    sorting(0,drives,drives_count);
   /*  int max=-1,sum=0;
     for(int i=0; i<keyboards_count; i++)
     {
         for(int j=0; i<drives_count; j++)
         {
     ...
     for(int i = 0, j = 0; i < keyboards_count; i++){
            for(; j < drives_count; j++){
                sum=keyboards[i]+drives[j];
                printf("%d %d %d\n",i,j,sum);
                if(sum > b) break; //This prevents j from incrementing
                if(sum > max)
                    max = sum;
